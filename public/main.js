...
function getPlaceLatitude(place) {
  return Number(place.latitude ?? place.LATITUDE ?? place.lat ?? place.y);
}

function getPlaceLongitude(place) {
  return Number(place.longitude ?? place.LONGITUDE ?? place.lng ?? place.x);
}

function getPlaceName(place) {
  return place.name ?? place.place_name ?? place.KOR_NM ?? "이름 없음";
}

function getPlaceCategory(place) {
  return place.category ?? place.category_name ?? place.CATEGORY ?? "정보 없음";
}

function getPlaceCrowdLevel(place) {
  return place.crowdLevel ?? place.density ?? place.DENSITY ?? "정보 없음";
}
